...
%% this function returns a min-plus expansion of the function fntofit around xhat. It does this by using a 
...
% This can be changed for other functions of interest.
  ...
  xhat = xhat(2); % in the example used only the 2nd state influences the output directly (since y = ((x_2) ^3)/40
	...
	for(k = 1:length(centerPts)-1)
		...
		fnew= f';
		H = 2*A'*A;
		pval = fmincon(@(x) ((1/2)*x'*H*x + fnew*x), [3/40;mean(diff(yval))/mean(diff(xval));mean(yval)],...
