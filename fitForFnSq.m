...
% fitForFn(fntofit, xhat, numberOfQuadratics, numOfPtsAroundEachQuadratic, rangeAroundXhat );
...
% for + - x^3/40
%         pval = fmincon(@(x) ((1/2)*x'*H*x + fnew*x), [3/40;mean(diff(yval))/mean(diff(xval));mean(yval)],[],[],[],[],[.01;-250;-250],[],@(x)fnnonlincon(x,xsamplePts,fnvalatxsamplepts));
...
%      ver 1 : second derivative: 6*xhat^3/(40^2
        pval = fmincon(@(x) ((1/2)*x'*H*x + fnew*x), [(30/40^2)*(mean(xval))^4;mean(diff(yval)./diff(xval));fntofit(mean(xval))],[],[],[],[],[.01;-250;-250],[],@(x)fnnonlincon(x,xsamplePts,fnvalatxsamplepts),opt1);
